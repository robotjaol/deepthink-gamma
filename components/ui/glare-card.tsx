...
export const GlareCard = ({
  ...
  const backgroundStyle = {
    ...
    "--rainbow":
      theme === 'light'
        ? `repeating-linear-gradient(0deg, rgba(174,214,241,0) calc(var(--step) * 1), rgba(174,214,241,0.5) calc(var(--step) * 2), rgba(174,214,241,0) calc(var(--step) * 3))`
        : `repeating-linear-gradient(0deg, rgba(212,175,55,0) calc(var(--step) * 1), rgba(212,175,55,0.2) calc(var(--step) * 2), rgba(212,175,55,0) calc(var(--step) * 3))`,
    "--diagonal":
      theme === 'light'
        ? `repeating-linear-gradient(128deg, #F0F7FF 0%, #AED6F1 3.8%, #AED6F1 4.5%, #AED6F1 5.2%, #F0F7FF 10%, #F0F7FF 12%) var(--bg-x) var(--bg-y)/300% no-repeat`
  ...
  return (
    ...
    >
      <div className="h-full grid will-change-transform origin-center transition-transform duration-[var(--duration)] ease-[var(--easing)] delay-[var(--delay)] [transform:rotateY(var(--r-x))_rotateX(var(--r-y))] rounded-[var(--radius)] border border-slate-300 dark:border-slate-800 hover:[--opacity:0.6] hover:[--duration:200ms] hover:[--easing:linear] hover:filter-none overflow-hidden">
        <div className="w-full h-full grid [grid-area:1/1] mix-blend-soft-light [clip-path:inset(0_0_0_0_round_var(--radius))]">
          <div className={cn("h-full w-full bg-light-secondary dark:bg-dark-secondary", className)}>
            {children}
          </div>
        </div>
